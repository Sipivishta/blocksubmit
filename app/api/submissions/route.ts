...
import { computeSimilarityForSubmission } from '@/lib/similarity-pipeline';
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (assignmentError || !assignment) {
      // Also covers the teacher-student link requirement: the "assignments:
      // read access" RLS policy (0006_teacher_student_links.sql) makes an
      // assignment invisible to a student who isn't linked to its teacher,
      // so an unlinked student's assignment lookup lands here exactly as
      // if the assignment didn't exist — the same message on purpose, so
      // this route never confirms an assignment exists for a teacher the
      // student isn't authorized to see.
    ...
    // ---- Similarity analysis (independent of the state machine) ----
    // Deliberately NOT a prerequisite for Step 4 below: this never
    // changes `status`, never throws into this route's control flow (its
    // own internals already catch everything, and this await is wrapped
    // again here as a second safety net), and a failure here has no
    // effect on whether the submission reaches CONFIRMED.
    await computeSimilarityForSubmission({
      id: submissionId,
      assignment_id: assignmentId,
      file_path: key,
      mime_type: validatedMimeType
    }).catch((err) => console.error('Similarity pipeline threw unexpectedly', err));
